...
public abstract class IDatabaseQuery {
    ...
    public abstract List<TableInfo> queryTables(Set<ColumnOverride> columnOverrides);
    ...
    public static class DefaultDatabaseQuery extends IDatabaseQuery {
        ...
        @Override
        public List<TableInfo> queryTables(Set<ColumnOverride> columnOverrides) {
            boolean isInclude = strategyConfig.getInclude().size() > 0;
            boolean isExclude = strategyConfig.getExclude().size() > 0;
            //所有的表信息
            List<TableInfo> tableList = new ArrayList<>();
            ...
            try {
                ...
                // 性能优化，只处理需执行表字段 github issues/219
                tableList.forEach(a -> convertTableFields(a, columnOverrides));
                return tableList;
            } catch (SQLException e) {
                throw new RuntimeException(e);
            } finally {
        ...
        private void convertTableFields(TableInfo tableInfo, Set<ColumnOverride> columnOverrides) {
            DbType dbType = this.dataSourceConfig.getDbType();
            String tableName = tableInfo.getName();
            try {
                final Map<String, DatabaseMetaDataWrapper.ColumnsInfo> columnsMetaInfoMap = new HashMap<>();
                ...
                dbQuery.query(tableFieldsSql, result -> {
                    ...
                    ColumnOverride columnOverride = columnOverrides.stream()
                            .filter(a -> StrUtil.equals(a.getColumnName(), columnName))
                            .findAny().orElse(null);
                    String propertyName = null;
                    if (ObjectUtil.isNotNull(columnOverride)) {
                        propertyName = columnOverride.getJavaProperty();
                    } else {
                        propertyName = entity.getNameConvert().propertyNameConvert(field);
                    }

                    IColumnType columnType = dataSourceConfig.getTypeConvert()
                            .processTypeConvert(globalConfig, field);
                    field.setPropertyName(propertyName, columnType);
                    field.setMetaInfo(new TableField.MetaInfo(columnsMetaInfoMap.get(columnName.toLowerCase())));
                    tableInfo.addField(field);
                });
